...
The measurement `arrange.py` needs and `analysis.py` does not provide: tempo
...
empty rather than wrong, and `arrange.py` reads that as "play a rhythm section
...
def detect(audio: np.ndarray, track: Track, sample_rate: int = ANALYSIS_RATE) -> Chart:
    ...
    chart with no rhythm to it is not something `arrange` could play. A track
    ...
    if confidence < MIN_CONFIDENCE:
        # Measured, and not good enough to play. `arrange` renders a rhythm
